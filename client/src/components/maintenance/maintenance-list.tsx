...
export default function MaintenanceList({ 
  ...
  const { data: maintenanceTasks, isLoading: tasksLoading } = useQuery<Maintenance[]>({
  ...
  if (filter === "pending") {
    filteredTasks = maintenanceWithDetails.filter(task => task.status === "pending");
  } else if (filter === "overdue") {
    filteredTasks = maintenanceWithDetails.filter(task => task.status === "overdue");
  } else if (filter === "completed") {
    filteredTasks = maintenanceWithDetails.filter(task => task.status === "completed");
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        {viewAll && (
          <a href="/maintenance" className="text-primary text-sm flex items-center">
            View All
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                  ...
                  <tr key={task.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`text-sm font-medium ${
                        ...
                        isAfter(new Date(), new Date(task.dueDate)) ? "text-red-500" : 
                        "text-gray-900"
                      }`}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs rounded-full ${getStatusBgColor(task.priority)}`}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        <img 
                          src={task.assignedToUser?.profileImage || "https://via.placeholder.com/40"} 
                ...
                <tr>
                  <td colSpan={showActions ? 6 : 5} className="px-4 py-3 text-center text-gray-500">
