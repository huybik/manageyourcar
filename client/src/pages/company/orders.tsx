...
function CreateOrderForm({ onCancel, onSuccess, parts, users }: CreateOrderFormProps) {
  const [supplier, setSupplier] = useState("");
  const [selectedParts, setSelectedParts] = useState<Array<{partId: number, quantity: number, price: number}>>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedParts.length === 0) {
      toast({
        title: "Cannot create empty order",
        description: "Please add at least one part to the order",
        variant: "destructive",
      });
      return;
    }
    
    const totalAmount = selectedParts.reduce((sum, item) => sum + (item.quantity * item.price), 0);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              <tr className="bg-gray-50">
                ...
                <td className="px-4 py-2 text-sm font-medium text-gray-900">
                  ${selectedParts.reduce((sum, item) => sum + (item.quantity * item.price), 0).toFixed(2)}
                </td>
                <td></td>
              </tr>
            </tbody>
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        >
          {createOrderMutation.isPending && (
            <span className="material-icons animate-spin mr-2" style={{ fontSize: '18px' }}>refresh</span>
          )}
          Create Order
        </Button>
      </div>
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage part orders and suppliers</p>
        </div>
              ...
              {ordersLoading ? (
                ...
                filteredOrders.map(order => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{formatDate(order.createdDate)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-gray-500">
                    {searchTerm || statusFilter !== "all" ? 'No orders matching your search criteria' : 'No orders available'}
                  </td>
                </tr>
              )}
            </tbody>
