...
export const insertVehiclePartSchema = createInsertSchema(vehicleParts, {
  ...
  // isCustom is not in the base table definition, so it shouldn't be here unless added to pgTable
}).omit({
  id: true,
});
...
export const insertMaintenanceSchema = createInsertSchema(maintenance, {
  ...
  completedDate: z.date().or(z.string().datetime()).optional().nullable(), // Allow setting completedDate
}).omit({
  id: true,
  // completedDate: true, // completedDate is set when status changes to 'completed' - REMOVED this omit
});
...
export const insertOrderSchema = createInsertSchema(orders, {
  ...
  orderedDate: z.date().or(z.string().datetime()).optional().nullable(), // Add orderedDate
  receivedDate: z.date().or(z.string().datetime()).optional().nullable(), // Add receivedDate
  totalAmount: z.number().positive().optional().nullable(),
}).omit({
  id: true,
  orderNumber: true, // Generated by the server
  // orderedDate: true, // Set later - REMOVED this omit
  // receivedDate: true, // Set later - REMOVED this omit
});
