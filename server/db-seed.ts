...
export async function seedDatabase() {
  ...
  const [sedan] = await db.insert(vehicles).values({
    name: "Company Sedan",
    type: "sedan",
  ...
  const [van] = await db.insert(vehicles).values({
    name: "Cargo Van",
    type: "van",
  ...
  // Create parts
  const [oilFilters] = await db.insert(parts).values({
    name: "Oil Filter",
    sku: "OF-123456",
  ...
  const [brakePads] = await db.insert(parts).values({
  ...
  const [wiperBlades] = await db.insert(parts).values({
  ...
  const [airFilters] = await db.insert(parts).values({
  ...
  const [oilChange] = await db.insert(maintenance).values({
    vehicleId: truck.id,
    type: "oil_change",
  ...
  const [brakeInspection] = await db.insert(maintenance).values({
    vehicleId: sedan.id,
  ...
  const [tireRotation] = await db.insert(maintenance).values({
    vehicleId: van.id,
    type: "tire_rotation",
